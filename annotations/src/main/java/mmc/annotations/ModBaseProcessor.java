...
//            annotationProperties.put("debug", "true");
    ...
    protected String getPackageName() {
        packageName = (rootPackageName = annotationsSettings(AnnotationSettingsEnum.rootPackage, () -> {
            Fi[] list = rootDirectory.child("core/src").list();
            if (list.length == 0)
                err("Cannot find rootPackage, please write rootPackage by using gradle plugin or @" + AnnotationSettings.class.getCanonicalName());
            return list[0].name();
        })) + ".gen";
        return packageName;
    }
...
//            System.out.println("supported->"+Arrays.toString(strings));
    ...
    public static void write(String packageName, String fileName, String content) throws IOException {
    ...
    public static void write(String fileName, String content) throws IOException {
        write(packageName, fileName, content);
    }
