...
public class MindustryGenerators {
    ...
    protected void run() {
        generate("splashes", this::splashes);
        generate("bubbles", this::bubbles);
        generate("cliffs", this::cliffs);
        generate("cracks", this::cracks);
        generate("block-icons", this::blockIcons);
        ...
        generate("scorches", this::scorches);
    }

    protected void disable() {
        generateSplashes = false;
        generateBubbles = false;
        generateCliffs = false;
        generateCracks = false;
        generateBlockIcons = false;
    ...
    protected void unitIcons() {
        ...
        content.units().each(type -> {
            ...
            try {
                ...
                for (Weapon weapon : type.weapons) {
                    weapon.load();
                    image.draw(weapon.flipSprite ? outline.get(get(weapon.region)).flipX() : outline.get(get(weapon.region)), (int) (weapon.x / Draw.scl + image.width / 2f - weapon.region.width / 2f), (int) (-weapon.y / Draw.scl + image.height / 2f - weapon.region.height / 2f), true);
                }
                ...
                for (Weapon weapon : type.weapons) {
                    ...
                    image.draw(wepReg, (int) (weapon.x / Draw.scl + image.width / 2f - weapon.region.width / 2f), (int) (-weapon.y / Draw.scl + image.height / 2f - weapon.region.height / 2f), true);
                }
