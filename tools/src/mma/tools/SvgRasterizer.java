...
public class SvgRasterizer{
    ...
    public static void main(String[] inputArgs){
        int[] sizes = new int[inputArgs.length];
        for(int i = 0; i < inputArgs.length; i++){
            sizes[i] = Strings.parseInt(inputArgs[i]);
        }
        /*
        ...
        **/
        ...
        Time.mark();
        Fi iconsFolder = Fi.get("svg_icons");
        iconsFolder.deleteDirectory();
        Fi iconsPartFolder = Fi.get("svg_icons_parts");
        Fi svgOutputFolder = Fi.get("svg_icons_parts");
        svgOutputFolder.deleteDirectory();
        iconsPartFolder.deleteDirectory();
        ...
        for(Fi img : list){
            if(img.extension().equals("png")){
                ...
                new SvgRasterizer().convert(new Pixmap(img), dst);
                dst.copyTo(iconsPartFolder.child(dst.name()));
                //dst.copyTo(svgOutputFolder.child(dst.name()));
                Main.main(new String[]{"-d",svgOutputFolder.absolutePath(),dst.absolutePath()});
                files.add(dst);
            }
        }
