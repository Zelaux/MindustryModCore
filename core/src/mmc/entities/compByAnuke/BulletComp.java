...
@mmc.annotations.ModAnnotations.MindustryEntityDef(value = { Bulletc.class }, pooled = true, serialize = false)
@Component(base = true)
abstract class BulletComp implements Timedc, Damagec, Hitboxc, Teamc, Posc, Drawc, Shielderc, Ownerc, Velc, Bulletc, Timerc {
    ...
    public boolean collides(Hitboxc other) {
        ...
        type.collides && (other instanceof Teamc t && t.team() != team) && !(other instanceof Unit f && !f.checkTarget(type.collidesAir, type.collidesGround)) && !(type.pierce && hasCollided(other.id())) && stickyTarget == null;
    }
    ...
    public boolean checkUnderBuild(Building build, float x, float y) {
        ...
        !build.block.underBullets || // bullet type allows hitting under bullets
        (aimTile != null && aimTile.build == build) || // same team has no 'under build' mechanics
        type.hitUnder || // a piercing bullet overshot the aim tile, it's fine to hit things now
    ...
    public void tileRaycast(int x1, int y1, int x2, int y2) {
        ...
        while (x >= 0 && y >= 0 && x < ww && y < wh) {
            ...
            if (build != null && isAdded() && checkUnderBuild(build, x * tilesize, y * tilesize) && build.collide(self()) && type.testCollision(self(), build) && !build.dead() && (type.collidesTeam || build.team != team) && !(type.pierceBuilding && hasCollided(build.id))) {
                if (type.sticky) {
                    if (build.team != team) {
